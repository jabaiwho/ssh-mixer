...
1. **No-change discovery:** Open the panel and confirm it does not start a Session. Verify clean Playback, Capture, and Output Monitor labels, including **Desktop (All)**, and confirm no temporary numeric source ID is persisted after refresh. Verify Sources opens by default; `j`/`k` enters each accordion section at its first option, closes the prior keyboard-opened section, and scrolls every focused row into view; `h`/`l` moves only among horizontal options. Confirm mouse-opened sections remain open together until clicked again and every clickable control has a visible idle border.
2. **Connection choices:** Save two disposable Connections, rename their Receivers, and select each for the next Session. Confirm rename does not change Connection identity or host trust and the normal panel/bar never displays a full Tailscale DNS name. Exercise the Connection type changed by the release. For Tailscale, confirm an offline/identity/address mismatch fails before SSH. For Direct SSH, confirm user OpenSSH configuration is not inherited. If OpenSSH Profile behavior changed, separately test Include/ProxyJump and explicit ProxyCommand review, plus `Match exec` rejection.
