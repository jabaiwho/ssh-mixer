...
1. **No-change discovery:** Open the panel and confirm it does not start a Session. Verify clean Playback, Capture, and Output Monitor labels, including **Desktop (All)**, and confirm no temporary numeric source ID is persisted after refresh. Verify `j`/`k`, `h`/`l`, arrows, mouse selection, and focused-row scrolling in the four panel views.
