...
Opening the panel, discovering sources, loading a Mix Profile, waking, unlocking, logging in, or reconnecting never starts audio. Direct Playback Input selection and playback-only Quick Start are explicit Start actions; Capture Input selection adds a separate confirmation before microphone audio starts.

**1. Inputs** is always visible above the accordion sections. The larger tinted, borderless accordion headers distinguish navigation sections from the thin outlined controls inside them. The current keyboard target has a bright gold/yellow fill and border that remains distinct from a saved or toggled selection. `j`/`k` or Up/Down moves to the adjacent visual row and lands on that row's leftmost visible enabled button, opening the next section at its first action; `h`/`l` or Left/Right moves only among buttons on the same visual row. Enter or Space activates the exact highlighted button. Number keys `1` through `7` jump directly to the corresponding section and highlight its first visible action. Entering a section scrolls its complete header and body into view whenever they fit; a taller section aligns at its top and then follows the exact target. Entering **1. Inputs**, including with `1`, returns to the absolute panel top so the panel header and Close button are visible. A section opened with the mouse remains open until clicked again, so several sections may be pinned open together.
...
Numeric PipeWire/PulseAudio IDs are runtime-only. **Selected** grants routing consent and a direct Playback Input selection starts the Session immediately. **Pinned** controls what remains visible and never selects, starts, or routes audio by itself. SSH-mixer keeps a protected, bounded local list of the 20 most recently observed Playback Sources so a user can reopen **Recently used**, pin a useful app, or clear the list. Recent history never auto-selects an app and never includes microphones. An explicitly selected Playback Source persists through its stable Source Matcher, remains visible while inactive, and may match several current application streams. During a Session, newly appearing matching streams attach automatically while unselected applications remain on the normal local output. Output Monitors still require an unambiguous device match. Capture matchers are recent-choice hints only and never reselect a microphone.

**Play chosen Inputs on** choices:

- **This PC:** no remote stream and no SSH-mixer route change.
- **Receiver:** selected playback is moved to the temporary mix and is not preserved locally for that Session. An Output Monitor cannot suppress the output it observes.
- **Both:** selected playback is streamed and looped back to its original output.

Capture is never looped into local speakers by SSH-mixer. A Capture Input requires confirmation before every start or restart that includes it. The prompt remains bound to the latest complete Input and Route Mode revision; an older in-flight operation cannot replace that choice while confirmation is visible. Changing a non-empty Input selection or Route Mode during an active Session performs one bounded stop/start transition and automatically applies the latest complete selection. A Route Mode-only change reuses the backend's persisted stable Source Matchers rather than resubmitting panel choice IDs, so changing the normal output device cannot turn an application selection into an Output Monitor. Turning off the final Input or choosing **End Stream** stops, clears the selection, and invokes ownership-safe cleanup.

For SSH and Both, one explicitly started Session may contain multiple bounded Stream Epochs. After 15 minutes, SSH-mixer waits for at least one second below -50 dBFS and then recreates only FFmpeg, SSH, and Receiver playback. It performs the same refresh at 30 minutes even without silence. Source selection, routing, active indication, and lifecycle consent remain in place, while remote output may have a brief gap. The sequential replacement runs without a terminal, display, notification, focus change, or overlapping playback pipeline. Process creation and the SSH handshake have a brief unavoidable resource cost. Silence detection runs inside the existing encoder rather than as another process; it adds a small continuous scan cost rather than a second background pipeline. Failure to create the replacement stops instead of retrying or falling back silently and appears in an urgent banner immediately below the panel header. The panel scrolls to a new error once; press `6` for detailed, locally redacted diagnostics.
...
Wake, unlock, login, source discovery, panel open, profile load, and network reconnection never start or resume a Session. Closing the panel does not stop a deliberately active Session; reopen it from the bar and choose **End Stream**.

## Mix Profiles and Quick Start

A Mix Profile stores a Connection, Route Mode, Source Matchers, privacy policy, bitrate, and timeout. Loading one opens the mixer and never starts it; use playback-only Quick Start, or turn off and reselect a reviewed Playback Input to begin. End Stream clears the loaded selection.
